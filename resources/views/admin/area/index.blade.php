@extends('admin.admin_layouts')
@section('admin_content')
<link rel="stylesheet" href="{{asset('public/backend/css/uniform.css')}}" />
<link rel="stylesheet" href="{{asset('public/backend/css/select2.css')}}" />

<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> </div>
  </div>
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="span12">
        <div class="widget-box ">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Union list table</h5>
          </div>
          <div class="widget-content nopadding respons-table">
            <table class="table table-bordered data-table responsive-table">
              <thead>
                <tr>
                  <th>Serial No.</th>
                  <th>Union Code</th>
                  <th>Union Name</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                @foreach($area as $key=>$row)
                <tr class="gradeX">
                  <td>{{ $key +1 }}</td>
                  <td style="text-align: center;">{{ $row->area_code }}</td>
                  <td style="text-align: center;">{{ $row->area_name }}</td>
                  <td style="text-align: center;">
                    <a href="{{ URL::to('admin/union/edit/'.$row->id) }} " class="btn btn-sm btn-info">Edit</a>
                    <a href="{{ URL::to('admin/union/delete/'.$row->id) }}" class="btn btn-sm btn-danger" id="delete">Delete</a>
                  </td>
                </tr>
                @endforeach
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
@endsection
